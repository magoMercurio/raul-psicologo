...
const Servicios = () => {
  return (
    ...
    >
      <h2 className="custom-block-big-title text-customLightBg3 flex flex-col justify-center items-center mb-10 mt-24 " > Mis Servicios</h2>
      ...
      <div className="grid grid-cols-1 justify-items-center md:grid-cols-2 lg:grid-cols-3 gap-10 ml-10 mr-10">
        <div className="lg:w-1/2 flex items-start" >
          <img src={imgList1} alt="imglist" className="w-60" />
        </div>
        <div> 
          <ul className="mt-6 mb-6 flex flex-col justify-start items-start gap-4">
          {
          servicesData.map((service, index) => (
            <li className="mb-4 flex  text-customLightBg3" key={`${service.title} - ${index}`}
            >
              <CheckArrowIcon />
        ...
        <div className="hidden md:flex  lg:w-1/2 items-end">
          <img src={imgList2} alt="imglist" className="w-60" />
        </div>
      </div>
